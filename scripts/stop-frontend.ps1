...
$backupPath = Join-Path $resolvedBackupDir "cloudfront-$DistributionId-before-disable-$timestamp.json"
...
Set-Content -Path $backupPath -Value $distributionConfigJson -Encoding utf8
Write-Host "Saved current CloudFront config to $backupPath"

$distributionConfigResponse = $distributionConfigJson | ConvertFrom-Json
if (-not $distributionConfigResponse.DistributionConfig.Enabled) {
  Write-Host "CloudFront distribution is already disabled."
  exit 0
}

$distributionConfigResponse.DistributionConfig.Enabled = $false
$updateConfigPath = Join-Path $env:TEMP "twf-cloudfront-disable-$DistributionId.json"
...
if ($Wait) {
  Write-Host "Waiting for CloudFront disablement to deploy..."
...
Write-Host ""
Write-Host "Frontend CloudFront distribution is disabling. The S3 bucket and files were kept for easy redeploy."
